...
Prefix commands:
> selfprefix {prefix} - sets your self prefix
> prefix or @greed - view your current prefix

Information commands:
> botinfo - sends information about greed
> ping - sends greed's ping
...
> softban (user) {reason} - bans user then unbans user from server
> unban (user) {reason} - unbans user from server
> mute (user) (time) {reason} - mutes user in server
> unmute (user) {reason}- unmutes user in server
> purge (amount) {user} - deletes a certian amount of messages
> nuke - deletes channel and makes a new one
> strip (user) {reason} - removes dangerous roles from user
